...
// Bloque 7: Ejecutar y Reparar traen su propio chat ligero (ver comentario de `ChatCorrida` en
// cada fichero) que reutiliza el mismo estado de corrida que la consola global de la banda 2, así
// que necesitan las mismas tres piezas que recibe `<ConsolaGlobal>` — de ahí que `contenidoPestana`
// pase a recibirlas y reenviarlas solo a esas dos pestañas.
function contenidoPestana(pestana: Pestana, corrida: EstadoCorridaGlobal) {
  switch (pestana) {
    ...
    case "Ejecutar":
      return (
        <Ejecutar corridaActiva={corrida.corridaActiva} eventos={corrida.eventos} marcarCorridaActiva={corrida.marcarCorridaActiva} />
      );
    case "Reparar":
      return (
        <Reparar corridaActiva={corrida.corridaActiva} eventos={corrida.eventos} marcarCorridaActiva={corrida.marcarCorridaActiva} />
      );
    case "Reports":
      return <Reports />;
  }
