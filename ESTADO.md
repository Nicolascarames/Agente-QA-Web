...
Actualizado: 2026-09-14 (probado el paquete tal cual se instalaría desde npm: un bug de seguridad
real —la barrera de escrituras nunca se ejecutaba— y credenciales en claro en los ficheros
generados, los dos corregidos; consola con bocadillos y gap; deuda técnica cerrada: resumen
multilínea, `dist-server/` ausente, restos del sistema anterior; snapshot acotado cerrado con
conclusión tras seis ciclos reales)
...
| El escenario debe aparecer en Redactar, editable, antes de escribir Page Objects/spec — **parcialmente conseguido (2026-09-14)** | `SKILL.md` §3 ahora pide escribir el `.feature` en disco antes de pedir confirmación y no repetir el Gherkin completo en la consola. Verificado en tres ciclos reales contra `pruebas/sauce/`: cuando el agente ve una elección de redacción real (qué frase exacta, qué producto), sigue prefiriendo `AskUserQuestion` con el Gherkin completo inline **antes** de escribir el fichero — la regla de la puerta 1 (no tocar `tests/pages/`/`tests/specs/` hasta confirmar) se respeta, pero la parte de "revísalo en Redactar" no se ejerce en la práctica porque `AskUserQuestion` resuelve la duda en el mismo turno sin pasar por el fichero. Anotado en `PROXIMOS-PASOS.md` para decidir si se fuerza a escribir siempre primero (incluso con ambigüedad) o se acepta esta vía como equivalente |
